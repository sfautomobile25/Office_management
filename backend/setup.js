...
if (dropTables) {
    const tables = ['users', 'system_config', 'audit_logs', 'website_content', 'seo_settings', 'security_logs', 'currency_rates'];
...
db.serialize(() => {
    ...
    db.run(`CREATE TABLE IF NOT EXISTS website_content (
        ...
        if (err) console.error('Error creating website_content table:', err.message);
    ...
    db.run(`CREATE TABLE IF NOT EXISTS currency_rates (
        ...
        if (err) console.error('Error creating currency_rates table:', err.message);
    ...
    db.parallelize(() => {
        ...
        bcrypt.hash(defaultPassword, 10, (err, hash) => {
            ...
            db.run(`INSERT OR IGNORE INTO users (username, email, password, role, permissions) 
                    VALUES (?, ?, ?, ?, ?)`,
                ['admin', 'admin@realestate.com', hash, 'admin', JSON.stringify(['all'])],
                (err) => {
        ...
        currencies.forEach(([base, target, rate]) => {
            db.run(`INSERT OR IGNORE INTO currency_rates (base_currency, target_currency, rate) 
                ...
                if (err) console.error(`Error inserting currency ${base}-${target}:`, err.message);
            });
        });
    });
});
