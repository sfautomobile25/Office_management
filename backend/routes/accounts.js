...
const checkAccountsPermission = (permissionType, resourceType) => {
  return async (req, res, next) => {
    ...
    if (user.role !== "accounts_officer" && user.role !== "manager") {
      return res
        .status(403)
        .json({ success: false, error: "Accounts access required" });
    }
...
function getRangeFromQuery(q) {
  ...
  if (period === "custom") {
    if (
      ...
      throw new Error(
        "Invalid custom date range (use start_date/end_date as YYYY-MM-DD)"
      );
    }
...
router.post(
  "/accounts",
  checkAccountsPermission("create", "accounts"),
  async (req, res) => {
    const {
      account_number,
      account_name,
    ...
    try {
      ...
      if (existing) {
        return res
          .status(400)
          .json({ success: false, error: "Account number already exists" });
      }
      ...
      await runAsync(
        ...
        [
          req.session.user.id,
          "CREATE_ACCOUNT",
          `Created account: ${account_number} - ${account_name}`,
        ]
      );
...
router.post(
  "/transactions",
  checkAccountsPermission("create", "transactions"),
  async (req, res) => {
    const {
      date,
      description,
    ...
    try {
      ...
      const transaction_id = `${prefix}-${timestamp}-${Math.floor(
        Math.random() * 1000
      )}`;
      ...
      const account = await getAsync("SELECT * FROM accounts WHERE id = ?", [
        account_id,
      ]);
      if (account) {
        const newBalance =
      ...
      await runAsync(
        ...
        [
          req.session.user.id,
          "CREATE_TRANSACTION",
          `Created transaction: ${transaction_id} - ${description}`,
        ]
      );
...
router.post(
  "/journal-entries",
  checkAccountsPermission("create", "journal_entries"),
  async (req, res) => {
...
router.post("/permissions", async (req, res) => {
  ...
  const { user_id, permission_type, resource_type, resource_id, expires_at } =
    req.body;
...
router.get(
  "/ledger/export",
  checkAccountsPermission("reports"),
  async (req, res) => {
    try {
      const { start, end, period, year, month, quarter } =
      ...
      for (const acc of accounts) {
        ...
        // Sheet name safe
        const sheetName = `${acc.account_number} ${acc.account_name}`.slice(
          0,
          28
        );
        ...
        for (const r of lines) {
          ...
          ws.addRow({
            ...
            desc: (r.line_description || r.entry_description || "").slice(
              0,
              300
            ),
            debit: d || "",
      ...
      res.status(400).json({
        success: false,
        error: e.message || "Failed to export ledger",
      });
    }
  }
);
...
router.get("/ledger", async (req, res) => {
  try {
    let { account_id, start_date, end_date } = req.query;

    if (!account_id) {
      return res
        .status(400)
        .json({ success: false, error: "account_id required" });
    }

    let accountIdNum = null;

    // 1) Try as accounts.id (numeric)
    if (/^\d+$/.test(String(account_id))) {
      const byId = await getAsync(`SELECT id FROM accounts WHERE id = ?`, [
        Number(account_id),
      ]);
      if (byId?.id) accountIdNum = byId.id;
    }

    // 2) If not found by id, try as account_number
    if (!accountIdNum) {
      const byNumber = await getAsync(
        `SELECT id FROM accounts WHERE account_number = ?`,
        [String(account_id)]
      );
      if (byNumber?.id) accountIdNum = byNumber.id;
    }

    if (!accountIdNum) {
      return res.json({ success: true, ledger: [], source: "transactions" });
    }

    // 1️⃣ Try posted journal entries
    let rows = await allAsync(
      ...
      `,
      [accountIdNum, start_date, end_date]
    );

    let source = "journal";

    // 2️⃣ Fallback → transactions
...
router.get(
  ...
  async (req, res) => {
    try {
      const { start, end, period, year, month, quarter } = getRangeFromQuery(
        req.query
      );
      ...
      res
        .status(400)
        .json({ success: false, error: e.message || "Failed to generate P&L" });
    }
  }
);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      ws.addRow([`Period: ${start} to ${end} (${period})`]);
      ws.addRow([
        `Generated (Dhaka): ${formatDhakaDateTime(new Date().toISOString())}`,
      ]);
      ws.addRow([]);

      ws.addRow(["REVENUE"]);
      ws.addRow(["Account No", "Account Name", "Amount (BDT)"]).font = {
        bold: true,
      };
      revenueRows.forEach((r) =>
        ws.addRow([r.account_number, r.account_name, Number(r.amount || 0)])
      );
      ...
      res
        .status(400)
        .json({ success: false, error: e.message || "Failed to export P&L" });
    }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      if (!/^\d{4}-\d{2}-\d{2}$/.test(as_of)) {
        return res
          .status(400)
          .json({ success: false, error: "as_of must be YYYY-MM-DD" });
      }
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to generate Balance Sheet" });
    }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      const addSection = (title, items, total) => {
        ...
        ws.addRow(["Account No", "Account Name", "Balance (BDT)"]).font = {
          bold: true,
        };
      ...
      res.status(400).json({
        success: false,
        error: e.message || "Failed to export Balance Sheet",
      });
    }
  }
);
