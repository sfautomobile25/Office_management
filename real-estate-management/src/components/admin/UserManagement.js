...
function UserManagement() {
  ...
  const updateMeInStorage = (patch) => {
    ...
    // optional: notify other components (sidebar) instantly if you listen to it later
  ...
  const handleUpdateUser = async (userId, updates) => {
    try {
      setError("");
      const res = await userManagementAPI.updateUser(userId, updates);

      // ✅ if you updated yourself, reflect it immediately in localStorage
      const me = getMe();
                      ...
                      return (
                        <label key={perm} className="permission-checkbox">
                          <input
                            ...
                            onChange={() =>
                              handlePermissionToggle(user.id, perm)
                            }
                          />
